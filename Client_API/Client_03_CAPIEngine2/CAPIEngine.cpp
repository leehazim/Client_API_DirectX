...
LRESULT CAPIEngine::WndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_PAINT:
    {
        ...
        /*TextOut(hdc, 0, 0, L"Test Text", lstrlen(L"Test Text"));
        TextOut(hdc, 0, 25, L"Hi Window", lstrlen(L"Hi Window"));
        TextOut(hdc, 0, 50, L"안녕하세요", lstrlen(L"안녕하세요"));

        LPCWSTR tString = L"WCHAR string";
        TextOut(hdc, 0, 75, tString, lstrlen(tString));
        ...
        Ellipse(hdc, 210, 100, 310, 200);
        wsprintf(str, L"잘 그렸나 확인을 해보시오");
        TextOut(hdc, 100, 210, str, lstrlen(str));*/
