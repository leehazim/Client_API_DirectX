...
public:
	virtual void OnCreate() override {
		...
		// test 
		/*m_TestTextures[0] = new CTexture();
		m_TestTextures[0]->LoadTexture(this->hInst, this->m_hDC, L"resources/bongbong_0.bmp");

		m_TestTextures[1] = new CTexture();
		m_TestTextures[1]->LoadTexture(this->hInst, this->m_hDC, L"resources/bongbong_1.bmp");*/
	}

	virtual void OnDestroy() override {

		/*for (int i = 0; i < 2; i++) {
			SAFE_DELETE(m_TestTextures[i]);
		}*/
	...
	/*	m_pEnemyAimed->Update(deltaTime);
		...
		//}
		//else {
		//	m_AnimTime = m_AnimTime + m_DeltaTime;
		//}


		//// test Anim Update
		//if(m_pActor->GetIsActive())
		//	this->DrawTexture(m_pActor->GetDisplayX(), m_pActor->GetDisplayY(), m_TestTextures[m_CurrentFrameIndex]);
		//// test Anim Render
...
public:
	...
	// Anim Test
	//CTexture* m_TestTextures[2] = { nullptr, nullptr };
	//
	//int m_TotalFrameCount = 2;		// 총 프레임 개수
	//int m_CurrentFrameIndex = 0;	// 프레임이 돌기 위해서 시간마다 갱신되는 렌더링 중인 스프라이트
	//// 실시간 애니메이션 렌더링 용
	//float m_AnimTime = 0.0f;		// 애니메이션 진행 시간 (틱)
	//float m_TimeInterval = 1.0f;	// 애니메이션 프레임간 시간
